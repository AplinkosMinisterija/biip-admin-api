...
import { App, User } from './auth.service';
    ...
    routes: [
      {
        ...
        whitelist: [
          ...
          'auth.refreshToken',
          'auth.remindPassword',
          'auth.changePasswordVerify',
          'auth.changePasswordAccept',
        ],
        aliases: {
          'POST /login': 'auth.login',
          'POST /remind': 'auth.remindPassword',
          'POST /refresh': 'auth.refreshToken',
          'POST /change/verify': 'auth.changePasswordVerify',
          'POST /change/accept': 'auth.changePasswordAccept',
        },
      },
      {
        path: '',
        ...
        aliases: {
          ...
          // groups
          'GET /groups': 'auth.groups.list',
          'GET /groups/:id': 'auth.groups.get',
          'PATCH /groups/:id': 'auth.groups.update',
          'POST /groups': 'auth.groups.create',
          'DELETE /groups/:id': 'auth.groups.remove',
          ...
          // apps
          'GET /apps': 'auth.apps.list',
          'GET /apps/:id': 'auth.apps.get',
          'POST /apps': 'auth.apps.create',
          'GET /apps/users': 'auth.apps.getUsersApp',
          'POST /apps/:id/generate': 'auth.apps.generateApiKey',

          // users
          'GET /users': 'auth.users.list',
          'GET /users/:id': 'auth.users.get',
          'PATCH /users/:id': 'auth.users.update',
          'POST /users': 'auth.users.create',
          'DELETE /users/:id': 'auth.users.remove',
...
export default class ApiService extends moleculer.Service {
  ...
  async authenticate(
    ...
    if (auth) {
      ...
      if (token) {
        try {
          const user = await ctx.call('auth.users.resolveToken', null, {
